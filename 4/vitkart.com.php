...
.timer {
  ...
  border-radius: 50%;
  border-top: 16px solid #0094B2;
  border-right: 16px solid white;
  border-bottom: 16px solid #0094B2;
...
$ret2=mysqli_query($con,"select * from subcategory1 where subcatid='$ids1' order by name asc limit 0,10");
while ($row2=mysqli_fetch_array($ret2)) 
{

?>            

<a href="subcategory1.php?id=<?php echo htmlentities($row2['id']) ?>"><?php echo htmlentities($row2['name']) ?></a>
<?php } ?>
</div>
...
<center>                                    <a onclick="categoryall();" >↓↓
                    ...
                    <div class="col-lg-3" id="categorymore" style="display:none;">
                        <div class="banner-category">
                            <div class="banner-category-head">
                                <i class="fas fa-bars"></i>
                                <span>top categories</span>
...
<?php } ?>
                            ...
                            <?php } ?>
                    <li>
<center>                                    <a onclick="categorynotall();" >↑↑
                                    </a>
</center></li>                                
                            </ul>
                        </div>
...
<script>
    function categoryall(){
        document.getElementById('categorymore').style.display='block';
        document.getElementById('categoryless').style.display='none';
    }
    function categorynotall(){
        document.getElementById('categorymore').style.display='none';
        document.getElementById('categoryless').style.display='block';
    }
</script>
...
$ret=mysqli_query($con,"select *,product.id as pid,productimage.image as pimg from product join productimage on productimage.productid=product.id  GROUP BY name ORDER BY date asc limit 8");
while ($row=mysqli_fetch_array($ret)) 
{
    # code...
                        ...
                        <div class="product-card <?php if($row['stock']=="Out of Stock") { echo "product-disable";}else{} ?>">
                            <div class="product-media">
                                <div class="product-label">
                                    <label class="label-text off"><?php $discount=(($row['discountprice']/$row['actualprice'])*100)-100; echo intval($discount); echo "%"; ?></label>
...
$ret=mysqli_query($con,"select *,product.id as pid,productimage.image as pimg from product join productimage on productimage.productid=product.id where product.featured='1'  GROUP BY name ORDER BY date asc limit 6");
while ($row=mysqli_fetch_array($ret)) 
{
    # code...
...
$ret=mysqli_query($con,"select *,product.id as pid,productimage.image as pimg from product join productimage on productimage.productid=product.id where product.new='1'  GROUP BY name ORDER BY date asc");
while ($row=mysqli_fetch_array($ret)) 
{
    # code...
...
$ret=mysqli_query($con,"select *,product.id as pid,productimage.image as pimg from product join productimage on productimage.productid=product.id where product.bestselling='1'  GROUP BY name ORDER BY date asc limit 10");
while ($row=mysqli_fetch_array($ret)) 
{
    # code...
...
$ret=mysqli_query($con,"select *,product.id as pid,productimage.image as pimg from product join productimage on productimage.productid=product.id where product.featured='1'  GROUP BY name ORDER BY date asc limit 10");
while ($row=mysqli_fetch_array($ret)) 
{
    # code...
