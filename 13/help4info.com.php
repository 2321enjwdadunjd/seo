...
$articles = Article::getPage($conn, $paginator->limit, $paginator->offset, true,$menuid);		// Show articles with page containing 5 article limit and 0 offset value
$top_articles = Article::getPage($conn, 100, 0, true,$menuid);	
$categories = Category::getPage($conn, $paginator->limit, $paginator->offset, true,$menuid);	


$paginator = new Paginator(isset($_GET['page']) ? $_GET['page'] : 1, 10, Article::getTotal($conn));	 // php Null coalescing operator [if $_GET['page'] exist then it will show else other] + Total number of content
...
<h1>Elon musk twitter news, chinese apps news , US news, science news, UK news, saudi arabia news, Historical news, gaming.</h1>
<div class="row">
 <div class="col-md-8">
			...
			<?php else: ?>
				<ul id="index">
					...
					<?php foreach ($articles as $article): ?>
						<li>
							<article class="">
								<h2><a href="/<?= $article['id']; ?>/<?= htmlspecialchars(str_replace(' ','-',str_replace('%','per',trim($article['url'])))); ?>"><?= htmlspecialchars($article['title']); ?></a></h2>
   ...
   <div class="col-md-4">
    ...
    <div class="card my-4">
     <h5 class="card-header">Gaming  </h5>
	 <div class="card-body">
	...
	<div class="card my-4">
     <h5 class="card-header"> Top 100 articles </h5>
	 

      
							<?php foreach ($top_articles as $article): ?>
								
										<a href="/<?= $article['id']; ?>/<?= htmlspecialchars(str_replace(' ','-',str_replace('%','per',$article['title']))); ?>"><?= htmlspecialchars($article['title']); ?></a>
          ...
          <div class="card my-4">
     <h5 class="card-header">Newspaper</h5>
	 <div class="card-body">
		Government proclamations, concerning royal ceremonies, laws, taxes, public health, and criminals, have been dubbed news since ancient times. Technological and social developments, often driven by government communication and espionage networks, have increased the speed with which news can spread, as well as influenced its content. The genre of news as we know it today is closely associated with the newspaper.U.S. News & World Report is a multifaceted digital media company dedicated to helping consumers, business leaders and policy officials make important decisions in their lives.
							
	 </div>					
	
    </div>
  <div class="card my-4">
     <h5 class="card-header"> Help4info Team </h5>
	 
     <div class="card-body">
      The content on help4info.com website is created by expert teams.
        </div>
    </div>
  ...
  <h3>software engineering, US can’t rule world alone, times of war history, mary VS elizabeth, rubber balls, hanseatic period, mexico discovery, NASA scientist, Russia, Joe Biden signs bill, Ancient Egyptian temple, US sanctions  block life-saving relief, Tongaunderwater,volcano impact, Israeliscientists, Nancy-Pelosi’s-Taiwan-trip, china warning, talibans afghanistan,Liz truss, Nancy Pelosi expected to visit Taiwan, scientific breakthrough, earth rotating fast, scorching heat, twitter-elon musk, Pelosi's visit Taiwan, piece of big Chinese space junk, one china policy, gaming effect on health, COVID-19, burned biomass, video gamers, US probes China's Huawei, ukraine news, saudi arabia news, solar flare alert, asteroids aka, RishiSunak, EU news, Uk leadership hunt, chinese apps.</h3>

  <!-- Side Widget -->
    <div class="card my-4">
     <h5 class="card-header"> Chinese apps </h5>
	 
     <div class="card-body">
      
       The government of India has ordered to ban 54 new Chinese apps, citing that these pose a security and privacy “threat” to users. Google Apps is a Web-based and collaborative Software as a Service (SaaS) solution that customises the proprietary Google platform and brand for businesses of all sizes, including large enterprises.
        </div>
    </div>

   </div>
   
   <h1>gaming , android apps ,  chinese smartphones app, google apps , best smartphones </h1>
