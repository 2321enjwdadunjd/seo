...
<style>
	...
	<section class="home">
		<div id="home">
			<div class="slideshow-container">
					<?php
				 $dat = date('Y-m-d');
					$query = mysqli_query($mysqli,"SELECT * FROM tbl_slider_banner WHERE status = '1' AND end_date >= '$dat'");
					while($get = mysqli_fetch_array($query)){
					?>
					<div class="mySlidess">
					  <img src="<? echo $main_url; ?><? echo $get['banner_image']; ?>" style="width:100%;height:500px">
					 	</div>
					<?}?>
	...
	<section class="" style="clear:both;">
		<div class="container" style="margin-top: 3%; clear:both;">
			<div class="col-md-9 col-sm-12 col-xs-12 padding_zero">
				<div class="col-md-12" style="margin-bottom: 2%;">
					<input type="text" class="form-control name_seac" placeholder="Search by name" />
					<div id="display_info"></div>
				</div>
				<form action="search-list.php" method="GET">
				<div class="col-md-3 col-sm-3 col-xs-6">
					<select class="form-control" id="sele" name="state">
					<option value="Select State">Select State</option>
					<?
						$state = mysqli_query($mysqli,"SELECT * FROM tbl_state ORDER BY name ASC");
						while($fetstate = mysqli_fetch_array($state)){
					?>
						
						<option value="<? echo $fetstate['id']; ?>"><? echo $fetstate['name']; ?></option>
				...
				<div class="col-md-2 col-sm-3 col-xs-6 city_cel">
				...
				<div class="col-md-3 col-sm-3 col-xs-6">
					<select class="form-control" name="category">
						...
						<?
						$cate = mysqli_query($mysqli,"SELECT * FROM tbl_category ORDER BY category_name ASC ");
						while($fetcate = mysqli_fetch_array($cate)){
						?>
						<option value="<? echo $fetcate['cid']; ?>"><? echo $fetcate['category_name']; ?></option>
						<?
						} ?>
					</select>
				</div>
				<div class="col-md-1 col-sm-5 col-xs-12 search_cel">
					<a href="javascript:void(0);">
						<input type="submit" class="scarch_icon" placeholder="Search" name="search" value="search"/>
					</a>
				</div>
				</form>
			</div>
			<div class="col-md-3 col-sm-5 col-xs-12 lates_lis">
				
				<h3 class="latest_list_h3">Latest Listing</h3>
			</div>
		</div>
	</section>
	...
	<section style="margin-top: 2%;">
		<div class="row">
			<div class="container">
				...
				<?
				$cat =mysqli_query($mysqli,"SELECT * FROM tbl_category WHERE status = '1' ORDER BY category_name ASC LIMIT 12");
				while($get_cat = mysqli_fetch_array($cat)){
				
				?>
					<div class="col-md-3 col-sm-6 col-xs-6">
						<a href="sub_category.php?id=<? echo $get_cat['cid']; ?>">
							<div class="categories_col">
								<div class="categories_list">
									<img src="<? echo $main_url; ?><? echo $get_cat['category_image']; ?>"/>
									<h5><? echo $get_cat['category_name']; ?></h5>
								</div>						
							</div>
						</a>
					</div>
					...
					<div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
						<div class="text-center">
							<a class="btn btn-outline-dark mt-auto btn_view_more" href="category.php">View more (Next page)</a>
						</div>
					</div>
				</div>
				<div class="col-sm-12 col-xs-12 list_mob" style="display:none;">
				
				<h3 class="latest_list_h3">Latest Listing</h3>
			</div>
				<div class="col-md-3 col-sm-12 col-xs-12">
					<div class="categories_col_letist">	
				
			
						
						<marquee class="" direction="up" height="608px" scrollamount="2" onMouseOver="this.stop()" onMouseOut="this.start()" loop="">
							<ul>
							<?
							$list =mysqli_query($mysqli,"SELECT * FROM tbl_post WHERE status = '1' AND active= '1' ORDER BY date_time DESC LIMIT 12");
							while($get_list = mysqli_fetch_array($list)){
							$img = $get_list['image_1'];
							
							?>
								<li>
									<a href="listing_details.php?lid=<? echo $get_list['id'];?>">
										<div class="marquee_matter">
											<div class="col-md-3 col-sm-4 col-xs-4" style="padding:0;">
												<img src="<?  echo $main_url; ?><? if($img != ""){ echo $get_list['image_1'];}else{ echo "bg.jpg";} ?>" style="width:100%;"/>
											</div>
											<div class="col-md-9 col-sm-8 col-xs-8" style="padding:0;">
												<p><? echo $get_list['title']; ?></p>
											</div>
										</div>
									</a>
								</li>
...
<script>
$(".name_seac").keyup(function(){

 var name=$(".name_seac").val();
if(name == ""){
$( '#display_info' ).hide();
}else{
$( '#display_info' ).show();
