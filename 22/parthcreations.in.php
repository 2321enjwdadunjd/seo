        ...
        <main class="main">
                ...
                <div class="container">
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="intro-slider-container slider-container-ratio slider-container-1 mb-2 mb-lg-0">
                                <div class="intro-slider intro-slider-1 owl-carousel owl-simple owl-light owl-nav-inside" data-toggle="owl" data-owl-options='{"nav": false,"768": {"nav": true}}}'>
                                    <?php
                                    $sliderQuery = $conn->query('SELECt * FROM `slider` ');
                                    while($sliderData = mysqli_fetch_array($sliderQuery)){
                                    ?>
                                    <div class="intro-slide">
                                            <img src="admin/assets/img/slider_image/<?=$sliderData['image'];?>" alt="Image Desc">
                                        
                                        <?php if($sliderData['slidertitle'] || $sliderData['btnname']){?>
                                        <div class="intro-content">
                                            <?php if($sliderData['slidertitle']){?>
                                                <h3 class="intro-subtitle">Topsale Collection</h3>
                                                <h1 class="intro-title"><?=ucwords($sliderData['slidertitle']);?></h1>
                                            <?php } 
                                             if($sliderData['btnname']){?>
                                            <a href="category" class="btn btn-outline-white">
                                                <span><?=strtoupper($sliderData['btnname']);?></span>
                    ...
                    <div class="owl-carousel owl-simple" data-toggle="owl" 
                        ...
                        <?php 
                            ...
                            while($discountData = mysqli_fetch_array($discountQuery)){ ?>
                        <div>
                            <a href="product?cat=<?=base64_encode($discountData['id']);?>" class="brand">
                                <img src="admin/assets/img/blog/<?=$discountData['image'];?>" class="catImage" alt="Brand Name" style="width: 120px !important;">
                            </a>
                            <p class="discountText"><?=$discountData['discount'];?>% Off</p>
                        </div>
                    ...
                    <div class="tab-pane p-0 fade show active" role="tabpanel" aria-labelledby="trendy-all-link">
                        <div class="owl-carousel owl-simple carousel-equal-height carousel-with-shadow" data-toggle="owl" 
                            data-owl-options='{
                             ...
                             <?php
                            $proQuery = $conn->query('SELECT * FROM `product` ');
                            while($proData = mysqli_fetch_array($proQuery)){
                                $imgExt = explode(',' ,$proData['image']);
                            ?>
                            <div class="product product-11 text-center">
                                <figure class="product-media">
                                    <a href="single-product?pro=<?=base64_encode($proData['id']);?>">
                                        <?php
                                        foreach($imgExt as $imgName){
                                             echo   '<img src="admin/assets/img/product/'.$imgName.'" alt="Product image" class="product-image image-full-fit">';
                                            //  echo   '<img src="admin/assets/img/product/'.$imgName.'" alt="Product image" class="product-image-hover">';
                                            break;
                                        }
                                        ?>
                                    </a>
                                    ...
                                    <div class="product-action-vertical">
                                         <?php
                                            if(isset($_SESSION['parthUser'])){
                                                $wish = 'javascript:;';
                                                $user_id = $_SESSION['parthUser'];
                                                
                                            }else{
                                               $wish = 'login';
                                                $user_id = 0;
                                            }
                                            $sel = $conn->query("SELECT * FROM `whish_list` WHERE `user_id` = '$user_id' ");
                                            
                                            $selDate = mysqli_fetch_array($sel);
                                            $proselData = explode(',' ,$selDate['product_id']);
                                            if(in_array($proData['id'],$proselData)){?>
                                            
                                            <a href="<?=$wish;?>" class="btn-product-icon btn-product-wishlist btn-wishlist" <?php if($user_id != ''){?>onClick="addToWishlist('remove','<?=$proData['id'];?>');"<?php } ?>  title="Remove from wishlist"><span>Remove from wishlist</span></a>
                                                
                                        <?php }else{ ?>
                                        
                                            <a href="<?=$wish;?>" class="btn-product-icon btn-wishlist" <?php if($user_id != ''){?>onClick="addToWishlist('add','<?=$proData['id'];?>');"<?php } ?>  title="Add to Wishlist"><span>Add to Wishlist</span></a>
                                        <?php } ?>
                                ...
                                <div class="product-body">
                                    <h3 class="product-title"><a href="single-product?pro=<?=base64_encode($proData['id']);?>"><?=ucwords($proData['name']);?></a></h3><!-- End .product-title -->
                                    <div class="product-price">₹<?=$proData['price'];?></div><!-- End .product-price -->
                                </div><!-- End .product-body -->
                                <!--<div class="product-action">-->
                                     <?php 
                                            // if(isset($_SESSION["cart_item"])){
                                            //     $itemcheck = 0;
                                            //     foreach ($_SESSION["cart_item"] as $itemss){
                                            //         if(in_array($proData["id"],$itemss)) {
                                                    ?>
                                            ...
                                            // if($itemcheck != 1){
                                            ?>
                                                <!--<a href="javascript:;" class="btn-product btn-cart" id="add_<?=$proData['id']; ?>" onClick="cartAction('add','<?=$proData['id']; ?>')"><span>add to cart</span></a>-->
                                           <?php // }?>
                                <!--</div>-->
                            </div><!-- End .product -->
                            <?php } ?>
        		...
        		<div class="row">
        		    <?php
        				...
        				<div class="banner banner-display banner-link-anim">
                			<a href="#">
                				<img src="admin/assets/img/blog/<?=$categoryData['image'];?>" alt="Banner">
                			</a>

                			<div class="banner-content banner-content-center">
                				<h3 class="banner-title text-white"><a href="#"><?=$categoryData['name'];?></a></h3>
                				<a href="product?cat=<?=base64_encode($categoryData['id']);?>" class="btn btn-outline-white banner-link">Shop Now<i class="icon-long-arrow-right"></i></a>
                			</div>
            			</div>
        			</div>
                    <?php } ?>
            ...
            <div class="container">
                    ...
                    <div class="tab-pane p-0 fade show active" id="top-all-tab" role="tabpanel" aria-labelledby="top-all-link">
                        <div class="products">
                            <div class="row justify-content-center">
                               <?php
                                $proQuery = $conn->query('SELECt * FROM `product` ORDER BY id DESC LIMIT 0,8');
                                while($proData = mysqli_fetch_array($proQuery)){
                                    $imgExt = explode(',' ,$proData['image']);
                                ?>
                                    ...
                                    <div class="product product-11 mt-v3 text-center">
                                        ...
                                        <div class="product-body">
                                            ...
                                            <div class="product-nav product-nav-dots">
                                              <?php 
                                                $color_array = explode(',',$proData['color']);
                                                $colorquery = $conn->query("SELECT * FROM `color`");
                                                while($colordata=mysqli_fetch_array($colorquery)){ 
                                                    if(in_array($colordata['id'], $color_array)){
                                                        echo '<a href="#" class="active" style="background: '.$colordata['color_code'].';"><span class="sr-only">'.$colordata['name'].'</span></a>';
                                                    }
                                                }?>
                                            </div><!-- End .product-nav -->
                                        </div><!-- End .product-body -->
                ...
                <div class="more-container text-center">
                    <a href="product" class="btn btn-outline-darker btn-more"><span>Load more products</span><i class="icon-long-arrow-down"></i></a>
            ...
            <div class="container">
            	...
            	<div class="row justify-content-center">
            	    <?php
                       ...
                       $sql1=mysqli_query($conn,"SELECT * FROM payment_refund");
                    ...
                    <div class="col-lg-4 col-sm-6">
                        <div class="icon-box icon-box-card text-center">
                            <span class="icon-box-icon">
                                ...
                                <?php }elseif($row1['id'] == 2){?>
                                 <i class="icon-rotate-left"></i>
                                 <?php }elseif($row1['id'] == 3){?>
                                  <i class="icon-life-ring"></i>
                                  <?php }?>
                            </span>
                            <div class="icon-box-content">
                                <h3 class="icon-box-title"><?=$row1['name'];?></h3>
            ...
            <!--            }'>-->
                        <?php
                            // $blogQuery = $conn->query('SELECt * FROM `blog` ORDER BY rand() LIMIT 3');
                            // while($blogData = mysqli_fetch_array($blogQuery)){
                        ?>
                        <!--<article class="entry entry-display">-->
                        <!--    <figure class="entry-media">-->
                        <!--        <a href="single-blog?id=<?=base64_encode($blogData['id']);?>">-->
                        <!--            <img src="admin/assets/img/blog/<?=$blogData['image'];?>" alt="image desc">-->
                        <!--        </a>-->
                        <!--    </figure>-->

                        <!--    <div class="entry-body pb-4 text-center">-->
                        <!--        <div class="entry-meta">-->
                        <!--            <a href="single-blog?id=<?=base64_encode($blogData['id']);?>"><?=date('M d, Y',strtotime($blogData['date']));?></a>-->
                        <!--        </div>-->

                        <!--        <h3 class="entry-title">-->
                        <!--            <a href="single-blog?id=<?=base64_encode($blogData['id']);?>"><?=$blogData['title'];?></a>-->
                        <!--        </h3>-->

                        <!--        <div class="entry-content">-->
                        <!--            <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Phasellus hendrerit.<br>Pelletesque aliquet nibh necurna. </p>-->
                        <!--            <a href="single-blog?id=<?=base64_encode($blogData['id']);?>" class="read-more">Read More</a>-->
                        <!--        </div>-->
                        <!--    </div>-->
                        </article>
                        <?php //} ?>
            ...
            <!--        <a href="blogs" class="btn btn-outline-darker btn-more"><span>View more articles</span><i class="icon-long-arrow-right"></i></a>-->
            <!--    </div>-->
            <!--</div>-->
        </main><!-- End .main -->
